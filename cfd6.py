...
for i in range(0, n_users):
    user[i].avg_r = mean([ ri for ri in  utility[i] if ri > 0]) if len(utility[i]) > 0 else 0
...
root = tk.Tk()
root.wm_title("Best Recommendation")
img = ImageTk.PhotoImage(Image.open("downloads/1"))
